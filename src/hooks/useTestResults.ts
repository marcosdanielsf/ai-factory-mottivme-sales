...
interface ValidationResult {
  ...
  // Formatos legados variados
  metricas_globais?: Record<string, number>;
  total_tokens?: number;
...
export const useTestResults = (dateRange?: DateRange) => {
  ...
  const fetchResults = useCallback(async () => {
    try {
      ...
      let avQuery = supabase
        .from("agent_versions")
        .select(
          "id, agent_name, version, status, is_active, created_at, updated_at, client_id, location_id, last_test_score, validation_score, total_test_runs, validation_result, deployment_notes, validation_status, system_prompt, business_config",
        )
      ...
      if (!avError && agentVersions && agentVersions.length > 0) {
        const mappedFromAV = (agentVersions as Record<string, any>[]).map(
          (av) => {
            ...
            if (
              vr.validator?.test_results &&
              vr.validator.test_results.length > 0
            ) {
            ...
            } else if (vr.resultados && vr.resultados.length > 0) {
              // Formato validação de cenários (Julia Amare, Isabella, etc.)
              testResults = vr.resultados.map((r) => ({
                name: String(r.cenario_nome || r.nome || "Cenário"),
                input: String(r.mensagem_teste || r.mensagem || ""),
                score: Number(r.nota) || 0,
                passed: r.aprovado === true,
                feedback: String(r.justificativa || ""),
                simulated_response: String(
                  r.resposta_simulada || r.resposta || "",
                ),
              }));
            } else if (vr.validacoes && vr.validacoes.length > 0) {
              // Formato validações (Isabella SDR)
              testResults = vr.validacoes.map((v) => ({
                name: String(v.nome || v.id || "Validação"),
                input: String(v.mensagem || ""),
                score: Number(v.nota) || 0,
                passed: v.atende_expectativa === true,
                feedback: "",
                simulated_response: String(v.resposta || ""),
              }));
              ...
              testResults = vr.e2e_results.map((e2e) => {
                ...
                const leadMsgs = conversation.filter(
                  (m) => m.role === "user" || m.role === "lead",
                );
                const agentMsgs = conversation.filter(
                  (m) => m.role === "assistant",
                );
            ...
            const totalTokens = vr.totals?.total_tokens || vr.total_tokens || 0;
            const executionTimeMs =
              vr.totals?.total_time_ms ||
              (vr.duration_seconds ? vr.duration_seconds * 1000 : 0);
            ...
            const derivedDimensions = {
              tone:
                vr.scores?.tone ||
                vr.scores?.completeness ||
                Math.min(10, avgTestScore * 1.05),
              engagement: vr.scores?.engagement || Math.min(10, passRate),
              compliance: vr.scores?.compliance || Math.min(10, avgTestScore),
              accuracy:
                vr.scores?.accuracy || Math.min(10, avgTestScore * 0.95),
              empathy: vr.scores?.empathy || Math.min(10, avgTestScore * 0.9),
              efficiency:
                vr.scores?.efficiency ||
                vr.scores?.conversion ||
                Math.min(10, passedTests > 0 ? 8 : 4),
            };
            ...
            return {
              ...
              passed_tests:
                passedTests || (av.validation_status === "approved" ? 1 : 0),
              failed_tests:
                failedTests || (av.validation_status === "rejected" ? 1 : 0),
      ...
      if (!e2eError && e2eResults && e2eResults.length > 0) {
        ...
        const agentIds = [
          ...new Set(e2eResults.map((r) => r.agent_version_id)),
        ];
        ...
        const agentMap = new Map(
          (agentsData as Record<string, any>[] | null)?.map((a) => [a.id, a]) ||
            [],
        );
        ...
        for (const [key, scenarios] of executionGroups) {
          ...
          } as AgentTestRun & { e2e_scenarios?: Record<string, unknown>[] });
        }
      }
      ...
      try {
        ...
        console.info(
          "vw_latest_test_results query failed, continuing without it",
        );
      ...
      if (!trError && testResultsData && testResultsData.length > 0) {
        const mappedFromTR = testResultsData.map(
          (result: Record<string, any>) => {
            const overallScore = Number(result.overall_score) || 0;
            const passed = overallScore >= 8 ? 1 : 0;
            const failed = overallScore < 8 ? 1 : 0;

            return {
              id: result.id,
              agent_version_id:
                result.version ||
                result.agent_version_id?.slice(0, 8) ||
                "v1.0",
              status: "completed" as const,
      ...
      try {
        let rlQuery = supabase
          ...
          .select(
            "id, agent_version_id, overall_score, score_completeness, score_depth, score_tone, conversations_analyzed, strengths, weaknesses, action_taken, action_reason, created_at, execution_time_ms, agent_versions(agent_name, version, location_id)",
          )
          .order("created_at", { ascending: false })
          .limit(50);
        ...
        if (!rlError && rlData && rlData.length > 0) {
          const rlMapped = (rlData as Record<string, any>[]).map((rl) => {
            const agentInfo = (rl.agent_versions || {}) as Record<string, any>;
        ...
        console.info(
          "[useTestResults] reflection_logs query failed, continuing without it",
        );
      }
      ...
      if (allResults.length === 0) {
        ...
        if (!oldError && oldData && oldData.length > 0) {
          type OldRow = (typeof oldData)[number];
          const mapped = oldData.map((r: OldRow) => ({
            ...r,
            run_at: r.created_at,
      ...
      const uniqueResults = allResults
        .sort(
          (a, b) =>
            new Date(b.run_at || b.created_at).getTime() -
            new Date(a.run_at || a.created_at).getTime(),
        )
        .filter(
          (item, index, self) =>
  ...
  const deleteTestRun = useCallback(
    async (id: string, source?: string): Promise<boolean> => {
      try {
        setDeleting(id);

        // Verificar se é um registro E2E (id composto com underscore e ISO date)
        const isE2ERecord =
          id.includes("_") && id.includes("T") && id.includes("Z");
  ...
  const toggleVersionActive = useCallback(
    async (id: string, activate: boolean): Promise<boolean> => {
      try {
          ...
          prev.map((r) =>
            r.id === id
              ? {
                  ...r,
                  is_active: activate,
                  agent_status: activate ? "active" : "inactive",
                }
