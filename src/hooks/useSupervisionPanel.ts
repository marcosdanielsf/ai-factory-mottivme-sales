...
export const useSupervisionPanel = (): UseSupervisionPanelReturn => {
    ...
    try {
      ...
      if (filters.lastInteractionDays) {
        const dateXDaysAgo = new Date(
          Date.now() - filters.lastInteractionDays * 24 * 60 * 60 * 1000,
        ).toISOString();
      ...
      const uniqueConversations = data
        ? Array.from(
            new Map(
              (data as SupervisionConversation[]).map((c) => [
                c.conversation_id,
                c,
              ]),
            ).values(),
      ...
      console.log(
        `[SupervisionPanel] Carregadas ${data?.length || 0} conversas (${uniqueConversations.length} únicas)`,
      );
      ...
      if (
        ...
        getErrorMessage(err)?.includes("vw_supervision_conversations")
      ) {
        try {
          console.log(
            "[SupervisionPanel] Tentando fallback para view antiga...",
          );
          // Tenta view antiga
  ...
  const stats = useMemo(
    () => ({
      ...
      aiActive: conversations.filter(
        (c) => c.supervision_status === "ai_active",
      ).length,
      aiPaused: conversations.filter(
        (c) => c.supervision_status === "ai_paused",
      ).length,
      scheduled: conversations.filter(
        (c) => c.supervision_status === "scheduled",
      ).length,
      converted: conversations.filter(
        (c) => c.supervision_status === "converted",
      ).length,
    }),
    [conversations],
  );
...
function getMockConversations(): SupervisionConversation[] {
  ...
  return Array.from({ length: 15 }, (_, i) => ({
    ...
    supervision_notes:
      i % 3 === 0 ? "Lead qualificado, aguardando retorno." : null,
    scheduled_at:
      statuses[i % statuses.length] === "scheduled"
        ? new Date(Date.now() + 86400000).toISOString()
        : null,
    converted_at:
      statuses[i % statuses.length] === "converted"
