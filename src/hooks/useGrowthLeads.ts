...
export const useGrowthLeads = () => {
  const [kpis, setKpis] = useState<GrowthLeadsKPIs>({
    total: 0,
    withEmail: 0,
    withWhatsapp: 0,
    withLinkedin: 0,
  ...
  const fetchKPIsAndCharts = useCallback(async () => {
    ...
    try {
      ...
      const countryParam =
        filters.countries.length === 1 ? filters.countries[0] : null;
      ...
      if (filters.countries.length > 0) {
        countryData = countryData.filter((c) =>
  ...
  useEffect(() => {
    ...
    supabase.rpc("growth_leads_regions").then(({ data, error: err }) => {
      if (!err && data) {
        setAvailableRegions(
          (data as { region: string }[]).map((r) => r.region),
        );
      }
    });
  }, []);
  ...
  const fetchTable = useCallback(async () => {
    ...
    try {
      ...
      let query = supabase
        .from("growth_leads")
        .select(
          "id,name,phone,email,linkedin_url,whatsapp,instagram_username,company,region,country,title,source_channel,created_at,custom_fields",
          { count: "exact" },
        );
