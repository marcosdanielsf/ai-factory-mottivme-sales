  ...
  const handleImport = useCallback(async () => {
    ...
    for (const row of csvRows) {
      ...
      for (const mapping of activeMappings) {
        ...
        if (
          mapping.dbField === "ticket" ||
          mapping.dbField === "compare_price"
        ) {
          (product as unknown as Record<string, unknown>)[mapping.dbField] =
            parseFloat(value.replace(/[^\d.,]/g, "").replace(",", ".")) || 0;
        } else if (
          mapping.dbField === "sales_cycle_days" ||
          mapping.dbField === "target_units"
        ) {
          (product as unknown as Record<string, unknown>)[mapping.dbField] =
            parseInt(value) || 0;
        } else {
          ...
          {step === "upload" && (
            ...
            >
              ...
              <p className="text-xs text-zinc-600">
                Separador: virgula ou ponto-e-virgula. Encoding: UTF-8
              </p>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-zinc-400">
                  {csvRows.length} linhas encontradas. Mapeie as colunas:
                </p>
              </div>
              ...
              <div className="space-y-2">
                {mappings.map((mapping, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 bg-zinc-800/50 rounded-lg p-2.5"
                  >
                      ...
                      <tr className="border-b border-zinc-700">
                        {mappings
                          ...
                          .map((m, i) => (
                            <th
                              key={i}
                              className="text-left py-1 px-2 text-zinc-500 font-medium"
                            >
                              {m.dbField}
                            </th>
                          ))}
                        ...
                        <tr key={ri} className="border-b border-zinc-800">
                          {mappings
                            ...
                            .map((m, ci) => (
                              <td
                                key={ci}
                                className="py-1 px-2 text-zinc-300 truncate max-w-[150px]"
                              >
                                {row[m.csvColumn] || "-"}
                              </td>
                            ))}
          ...
          {step === "mapping" && (
            <>
              ...
              <button
                ...
                disabled={
                  importing || !mappings.some((m) => m.dbField === "name")
                }
              ...
              >
                ...
                {importing
                  ? "Importando..."
                  : `Importar ${csvRows.length} produtos`}
              </button>
            </>
          )}
          {step === "done" && (
...
function autoMapField(header: string): string {
  ...
  if (h.includes("nome") || h.includes("name") || h.includes("produto"))
    return "name";
  if (
    h.includes("preco") ||
    h.includes("price") ||
  ...
  if (h.includes("meta") || h.includes("target") || h.includes("unid"))
    return "target_units";
  if (h.includes("original") || h.includes("compare") || h.includes("de "))
