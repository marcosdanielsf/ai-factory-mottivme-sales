    ...
    >
      <div>
        <div className="flex items-center justify-between mb-1.5 md:mb-2">
          <h3 className="text-xs md:text-sm text-text-muted font-medium truncate pr-2">
            {title}
          </h3>
          {Icon && (
            <Icon
              size={14}
              className={`flex-shrink-0 md:w-4 md:h-4 ${isClickable ? "text-accent-primary" : "text-text-secondary"}`}
            />
          )}
        </div>
        <div className="flex items-end gap-1.5 md:gap-2">
          ...
          {trend && (
            <span
              className={`text-[10px] md:text-xs mb-0.5 md:mb-1 ${
                trendDirection === "up"
                  ? "text-accent-success"
                  : trendDirection === "down"
                    ? "text-accent-error"
