...
export const TestReportModal = ({ run, onClose }: TestReportModalProps) => {
  ...
  const totalResultsCount = hasE2EScenarios
    ? e2eScenarios.length
    : hasTestDetails
      ? testDetails.length
      : hasDebate
        ...
        <div className="p-4 border-b border-border-default flex items-center justify-between bg-bg-tertiary">
          <div className="flex items-center gap-4">
            ...
            {run.score_overall !== undefined && (
              <div
                className={`px-3 py-1.5 rounded-full border ${getScoreBgColor(run.score_overall)}`}
              >
                <span
                  className={`text-xs font-bold ${getScoreColor(run.score_overall)}`}
                >
        ...
        <div className="flex-1 overflow-auto p-6">
            ...
            <div className="max-w-5xl mx-auto space-y-4">
              ...
              {hasRealData ? (
                <div className="space-y-3">
                  ...
                  {isE2ETest &&
                    e2eScenarios.map((scenario, index) => {
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="text-left">
                                <p
                                  className={`font-bold text-sm uppercase tracking-wider ${passed ? "text-accent-success" : "text-accent-error"}`}
                                >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span
                                className={`text-xs px-2 py-1 rounded border ${getScoreBgColor(score)} ${getScoreColor(score)}`}
                              >
                                ...
                                {scenario.conversation.map((msg, msgIndex) => (
                                  <div key={msgIndex} className="flex gap-3">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <div
                                        className={`p-3 rounded-lg text-text-primary text-sm ${
                                          msg.role === "user" ||
                                          msg.role === "lead"
                                            ? "bg-bg-secondary"
                            ...
                            (!scenario.conversation ||
                              ...
                              <div className="px-4 pb-4">
                                <p className="text-sm text-text-muted italic">
                                  Detalhes da conversa não disponíveis para este
                                  cenário.
                                </p>
                              </div>
                            )}
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              <p
                                className={`font-bold text-sm uppercase tracking-wider ${test.passed ? "text-accent-success" : "text-accent-error"}`}
                              >
                          ...
                          <div className="flex items-center gap-3">
                            <span
                              className={`text-xs px-2 py-1 rounded border ${getScoreBgColor(test.score)} ${getScoreColor(test.score)}`}
                            >
                          ...
                          <div className="px-4 pb-4 space-y-4">
                            ...
                            <div className="p-3 bg-bg-primary/50 rounded border border-border-default">
                              <p className="text-[11px] text-text-secondary leading-relaxed">
                                <span
                                  className={`font-bold ${test.passed ? "text-accent-success" : "text-accent-error"}`}
                                >
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-text-muted">
                    Nenhum detalhe de teste disponível para esta execução.
                  </p>
                  <p className="text-text-muted text-sm mt-1">
                    Os dados de test_results não estão no validation_result.
                  </p>
                </div>
              )}
            </div>
          ...
          {activeTab === "prompt" && (
            <div className="max-w-5xl mx-auto space-y-6">
              ...
              {run.system_prompt ? (
                ...
                <div className="bg-bg-tertiary rounded-lg border border-border-default p-8 text-center">
                  ...
                  <p className="text-text-muted mb-2">
                    O prompt do agente não está disponível nesta visualização.
                  </p>
                  <p className="text-text-muted text-sm">
                    Para ver o prompt completo, acesse a página de Configuração
                    do Agente.
                  </p>
                </div>
              )}
              ...
              {run.business_config &&
                Object.keys(run.business_config).length > 0 && (
                  <div className="mt-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {Object.entries(run.business_config).map(
                        ([key, value]) => (
                          <div
                            key={key}
                            className="p-3 bg-bg-tertiary rounded border border-border-default"
                          >
                            <div className="text-[10px] uppercase text-text-muted mb-1">
                              {key.replace(/_/g, " ")}
                            </div>
          ...
          {activeTab === "conversation" && (
            <div className="max-w-5xl mx-auto space-y-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-text-primary">
                  {isE2ETest
                    ? "Conversas E2E Completas"
                    : "Raciocínio e Conversa de Teste"}
                </h3>
              </div>
              ...
              {hasRealData ? (
                <div className="space-y-4">
                  ...
                  {isE2ETest &&
                    e2eScenarios.map((scenario, index) => {
                      ...
                      return (
                        <div
                          key={scenario.id || index}
                          className="bg-bg-tertiary rounded-lg border border-border-default overflow-hidden"
                        >
                          <div
                            className={`p-3 border-b border-border-default ${passed ? "bg-accent-success/5" : "bg-accent-error/5"}`}
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`text-xs px-2 py-1 rounded ${getScoreBgColor(score)} ${getScoreColor(score)}`}
                                >
                                  Score: {score.toFixed(1)}/10
                                </span>
                                <span
                                  className={`text-xs px-2 py-1 rounded ${passed ? "bg-accent-success/10 text-accent-success" : "bg-accent-error/10 text-accent-error"}`}
                                >
                  ...
                  {hasTestDetails &&
                    testDetails.map((test, index) => (
                      ...
                      >
                        <div
                          className={`p-3 border-b border-border-default ${test.passed ? "bg-accent-success/5" : "bg-accent-error/5"}`}
                        >
                          <div className="flex items-center justify-between">
                            <span className="font-medium text-text-primary">
                            ...
                            <span
                              className={`text-xs px-2 py-1 rounded ${test.passed ? "bg-accent-success/10 text-accent-success" : "bg-accent-error/10 text-accent-error"}`}
                            >
                        ...
                        <div className="p-4 space-y-4">
                          ...
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1">
                              <div className="text-xs text-text-muted mb-1">
                                LLM-as-a-Judge (Análise)
                              </div>
                              <div
                                className={`p-3 rounded-lg text-sm border ${test.passed ? "bg-accent-success/5 border-accent-success/20 text-text-primary" : "bg-accent-error/5 border-accent-error/20 text-text-primary"}`}
                              >
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-text-muted">
                    Nenhuma conversa de teste disponível.
                  </p>
                  <p className="text-text-muted text-sm mt-1">
                    Os dados de conversação não estão presentes nesta execução.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="p-4 border-t border-border-default bg-bg-tertiary flex items-center justify-between">
          <div className="text-xs text-text-muted">
            {run.validation_status && (
              <span className="mr-4">
                Status:{" "}
                <span className="font-medium">{run.validation_status}</span>
              </span>
            )}
            ID: {run.id.slice(0, 8)}...
          </div>
