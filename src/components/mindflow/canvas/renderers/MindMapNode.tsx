...
export const MindMapNode = memo(
  ({ id, data, selected, sourcePosition, targetPosition }: NodeProps) => {
    const srcPos = (sourcePosition as Position) ?? Position.Right;
    const tgtPos = (targetPosition as Position) ?? Position.Left;
    ...
    const finishEdit = useCallback(() => {
      ...
      if (trimmed !== label) {
        updateElement(id, { data: { ...nodeData, label: trimmed } });
      }
      commitEdit();
    }, [id, draft, label, nodeData, updateElement, commitEdit]);
    ...
    return (
      <>
          ...
          >
            ...
            {isEditing ? (
              <input
                ...
                className="nodrag font-semibold text-sm leading-tight flex-1 bg-transparent border-none outline-none min-w-0"
                style={{ color }}
                autoFocus
              />
            ) : (
        ...
        {/* Source handle — dynamic based on layout */}
        <Handle
          type="source"
          position={srcPos}
          style={{ background: color, border: "none", width: 6, height: 6 }}
