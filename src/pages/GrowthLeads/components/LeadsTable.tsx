...
const thClass =
  "px-2 py-1.5 text-left text-[11px] font-medium text-text-muted whitespace-nowrap cursor-pointer hover:text-text-primary transition-colors select-none";
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (filters.enrichmentStatus === "enriched") {
        query = query.or(
          "email.not.is.null,whatsapp.not.is.null,instagram_username.not.is.null",
        );
      } else if (filters.enrichmentStatus === "no_contact") {
        query = query
          .is("email", null)
          .is("whatsapp", null)
          .is("instagram_username", null)
    ...
    switch (col) {
      case "name":
        return (
          <td
            key={col}
            className={`${tdClass} font-medium text-text-primary max-w-[180px] truncate`}
          >
            {lead.name}
          </td>
        );
      case "country":
        return (
          <td key={col} className={tdClass}>
            <span title={getCountryLabel(lead.country)}>
      ...
      case "region":
        return (
          <td key={col} className={`${tdClass} max-w-[100px] truncate`}>
            {lead.region ? lead.region : <NullCell />}
          </td>
        );
      case "company":
        return (
          <td key={col} className={`${tdClass} max-w-[130px] truncate`}>
            {lead.company ? lead.company : <NullCell />}
          </td>
        );
      case "title":
        return (
          <td key={col} className={`${tdClass} max-w-[160px] truncate`}>
            {lead.title ? lead.title : <NullCell />}
          </td>
        );
      case "source_channel":
        return (
                ...
                <tr key={i}>
                  {Array.from({ length: totalColSpan }).map((_, j) => (
                    <td key={j} className={tdClass}>
                      <div className="h-3 bg-bg-tertiary rounded animate-pulse w-16" />
                    </td>
                  ))}
                </tr>
              ))
                      ...
                      {lead.custom_fields &&
                        Object.keys(lead.custom_fields).length > 0 && (
                          ...
                          >
                            <ChevronDown
                              size={14}
                              className={`transition-transform ${expandedRow === lead.id ? "rotate-180" : ""}`}
                            />
                          </button>
                        )}
                    </td>
