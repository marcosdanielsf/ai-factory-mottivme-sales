...
export function Planejamento() {
  ...
  const {
    ...
    createGoal,
    refetch,
  } = useSalesGoals(effectiveLocationId);
  const funnelData = useSocialSellingFunnel(dateRange, effectiveLocationId);
  const {
    products: savedProducts,
    createProduct,
    updateProduct: updateSavedProduct,
  } = useProducts(effectiveLocationId);
  ...
  React.useEffect(() => {
    if (
      ...
      plan.products[0].name === "Produto Principal"
    ) {
      setPlan((p) => ({
        ...p,
  ...
  const handleSaveGoal = async () => {
    if (!effectiveLocationId || !dateRange.startDate || !dateRange.endDate)
      return;
    ...
    try {
      ...
      const totalBudgetPct = plan.marketing.subFunnels.reduce(
        (s, sf) => s + sf.pctBudget,
        0,
      );
      const weightedCpl =
        plan.marketing.subFunnels.length > 0 && totalBudgetPct > 0
          ? plan.marketing.subFunnels.reduce(
              (s, sf) => s + (sf.cpl * sf.pctBudget) / 100,
              0,
            )
          : 10;
      ...
      > = {
        ...
        ticket_medio_estimado:
          plan.products.length > 0
            ? plan.products.reduce((s, p) => s + p.ticket, 0) /
              plan.products.length
            : 1000,
        goal_conversion_rate:
          r.totalLeads > 0
            ? Math.round((r.totalSales / r.totalLeads) * 100)
            : 0,
        ...
        marketing_config: {
          ...
          scenarioResults: {
            pessimista: {
              totalSales: scenarios.pessimista.totalSales,
              totalRevenue: scenarios.pessimista.totalRevenue,
              roas: scenarios.pessimista.roas,
              netProfit: scenarios.pessimista.netProfit,
            },
            realista: {
              totalSales: scenarios.realista.totalSales,
              totalRevenue: scenarios.realista.totalRevenue,
              roas: scenarios.realista.roas,
              netProfit: scenarios.realista.netProfit,
            },
            otimista: {
              totalSales: scenarios.otimista.totalSales,
              totalRevenue: scenarios.otimista.totalRevenue,
              roas: scenarios.otimista.roas,
              netProfit: scenarios.otimista.netProfit,
            },
          },
        },
  ...
  const handleEditGoal = () => {
    if (activeGoal) {
      ...
      const prods =
        ...
        activeGoal.products_snapshot.length > 0
          ...
          : [
              {
                ...createDefaultProduct(),
                ticket: activeGoal.calc_average_ticket,
              },
            ];
      ...
      if (
        mktConfig?.subFunnels &&
        Array.isArray(mktConfig.subFunnels) &&
        mktConfig.subFunnels.length > 0
      ) {
        ...
        mktConfig?.trafegoSubFunnels &&
        Array.isArray(mktConfig.trafegoSubFunnels) &&
        mktConfig.trafegoSubFunnels.length > 0
      ) {
      ...
      const dailyBudget =
        mktConfig?.dailyBudget ??
        activeGoal.calc_daily_investment ??
        DEFAULT_STATE.marketing.dailyBudget;
      const savedScenario = mktConfig?.scenarioConfig as
        | ScenarioConfig
        | undefined;
  ...
  const actualData = useMemo(() => {
    const total =
      funnelData.socialSelling.leads +
      funnelData.trafego.leads +
      funnelData.whatsappDireto.leads +
      funnelData.organico.leads;
    return {
      leads: total,
      leadsSS: funnelData.socialSelling.leads,
      leadsTrafego: funnelData.trafego.leads,
      leadsOrganico:
        funnelData.whatsappDireto.leads + funnelData.organico.leads,
      responderam:
        funnelData.socialSelling.responderam +
        funnelData.trafego.responderam +
        funnelData.whatsappDireto.responderam +
        funnelData.organico.responderam,
      agendamentos:
        funnelData.socialSelling.agendaram +
        funnelData.trafego.agendaram +
        funnelData.whatsappDireto.agendaram +
        funnelData.organico.agendaram,
      comparecimentos:
        funnelData.socialSelling.compareceram +
        funnelData.trafego.compareceram +
        funnelData.whatsappDireto.compareceram +
        funnelData.organico.compareceram,
      vendas:
        funnelData.socialSelling.fecharam +
        funnelData.trafego.fecharam +
        funnelData.whatsappDireto.fecharam +
        funnelData.organico.fecharam,
    };
  }, [funnelData]);
  ...
  const projections = useMemo(() => {
    ...
    return {
      leads: calculateProjection(
        activeGoal.goal_leads_total,
        actualData.leads,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
      leadsSS: calculateProjection(
        activeGoal.goal_leads_social_selling,
        actualData.leadsSS,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
      leadsTrafego: calculateProjection(
        activeGoal.goal_leads_trafego,
        actualData.leadsTrafego,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
      leadsOrganico: calculateProjection(
        activeGoal.goal_leads_organico,
        actualData.leadsOrganico,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
      responderam: calculateProjection(
        activeGoal.goal_responderam,
        actualData.responderam,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
      agendamentos: calculateProjection(
        activeGoal.goal_agendamentos,
        actualData.agendamentos,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
      comparecimentos: calculateProjection(
        activeGoal.goal_comparecimentos,
        actualData.comparecimentos,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
      vendas: calculateProjection(
        activeGoal.goal_vendas,
        actualData.vendas,
        activeGoal.period_start,
        activeGoal.period_end,
      ),
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      const goalAccum = Math.round(
        (activeGoal.goal_leads_total / totalDays) * (i + 1),
      );
      const realAccum =
        dailyAccum[dateKey] || (i < daysElapsed ? actualData.leads : null);
      const projection =
        i >= daysElapsed
          ? Math.round(
              actualData.leads +
                projections.leads.currentDailyRate * (i - daysElapsed + 1),
            )
          : null;
      data.push({
        date: dateKey,
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                <RefreshCw
                  size={16}
                  className={`text-text-muted ${goalsLoading || funnelData.loading ? "animate-spin" : ""}`}
                />
              </button>
            </div>
          </div>
          ...
          <>
            ...
            <div className="bg-bg-secondary rounded-xl border border-border-default overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  {!showWizard && results.totalRevenue > 0 && (
                    <span className="text-xs text-text-muted ml-2">
                      {formatCurrency(results.totalRevenue, plan.currency)}{" "}
                      projetado
                    </span>
                  )}
                </div>
                <ChevronDown
                  size={18}
                  className={`text-text-muted transition-transform ${showWizard ? "rotate-180" : ""}`}
                />
              </button>
                ...
                <div className="border-t border-border-default">
                  ...
                  <div className="flex flex-col lg:flex-row">
                    ...
                    <div className="flex-1 px-6 py-5 lg:border-r border-border-default min-w-0">
                      <SegmentSelector
                        selectedSegment={plan.segment ?? null}
                        onApplyBenchmarks={(segmentKey) => {
                          const b =
                            SEGMENT_BENCHMARKS[segmentKey as SegmentKey];
                          if (!b) return;
                          setPlan((p) => ({
                            ...p,
                            segment: segmentKey,
                            products: p.products.map((prod, i) =>
                              i === 0
                                ? {
                                    ...prod,
                                    ticket: b.ticketAvg,
                                    salesCycleDays: b.salesCycleDays,
                                  }
                                : prod,
                            ),
                            marketing: {
                              ...p.marketing,
                              subFunnels: p.marketing.subFunnels.map((sf) => ({
                                ...sf,
                                cpl: b.cplAvg,
                                qualificationRate: b.qualificationRate,
                                schedulingRate: b.schedulingRate,
                      ...
                      {plan.step === 3 && (
                        <SalesStep
                          ...
                          onChange={(subFunnels) =>
                            setPlan((p) => ({
                              ...p,
                              marketing: { ...p.marketing, subFunnels },
                            }))
                          }
                        />
                      )}
                      ...
                      <div className="flex items-center justify-between mt-6 pt-4 border-t border-border-default">
                        <button
                          onClick={() =>
                            setPlan((p) => ({
                              ...p,
                              step: Math.max(1, p.step - 1) as 1 | 2 | 3,
                            }))
                          }
                          ...
                          {plan.step === 3 ? (
                            ...
                            <button
                              onClick={() =>
                                setPlan((p) => ({
                                  ...p,
                                  step: Math.min(3, p.step + 1) as 1 | 2 | 3,
                                }))
                              }
                      ...
                      {!effectiveLocationId && plan.step === 3 && (
                        <p className="text-xs text-yellow-400 mt-2">
                          Selecione um cliente para salvar a meta
                        </p>
                      )}
                    </div>
