...
export const Validation = () => {
  ...
  const filteredTestRuns = useMemo(() => {
    if (!activeLocationId) return testRuns;
    return testRuns.filter(
      (run) => (run as any).location_id === activeLocationId,
    );
  }, [testRuns, activeLocationId]);
  ...
  const clientGroups = useMemo(() => {
    ...
    filteredTestRuns.forEach((run) => {
      ...
      const clientName =
        LOCATION_NAMES[locationId] || (run as any).client_name || locationId;
    ...
    return Array.from(groups.values()).sort(
      (a, b) =>
        new Date(b.lastValidated).getTime() -
        new Date(a.lastValidated).getTime(),
    );
  }, [filteredTestRuns]);
  ...
  const stats = useMemo(() => {
    const approved = clientGroups.filter(
      (g) => g.latestStatus === "approved",
    ).length;
    const rejected = clientGroups.filter(
      (g) => g.latestStatus === "rejected",
    ).length;
    const avgScore =
      clientGroups.length > 0
        ? clientGroups.reduce((sum, g) => sum + g.latestScore, 0) /
          clientGroups.length
        : 0;
    ...
    <div className="max-w-6xl mx-auto p-8 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-bg-secondary border border-border-default rounded-lg p-4 hover:border-accent-error/30 transition-colors">
          ...
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-accent-error">
              {stats.rejected}
            </span>
            <span className="text-xs text-text-muted">
              (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-text-muted mt-0.5">
                      {group.agents.length} agente
                      {group.agents.length !== 1 ? "s" : ""} ·{" "}
                      {group.totalVersions} versões
                    </div>
                  </div>
                  ...
                  <div className="hidden md:flex items-center gap-6 text-sm">
                    ...
                    <div className="text-center min-w-[80px]">
                      ...
                      <div className="text-xs text-text-secondary">
                        {new Date(group.lastValidated).toLocaleDateString(
                          "pt-BR",
                        )}
                      </div>
                    </div>
                  ...
                  {/* Chevron */}
                  <div
                    className={`transition-transform ${expandedClient === group.locationId ? "rotate-90" : ""}`}
                  >
                ...
                {expandedClient === group.locationId && (
                  <div className="border-t border-border-default bg-bg-tertiary/30">
                    {group.agents.map((agent) => {
                      ...
                      const activeVersion = agent.versions.find(
                        (v: any) => v.is_active,
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {/* Chevron */}
                            <div
                              className={`transition-transform ${expandedAgent === agentKey ? "rotate-90" : ""}`}
                            >
                              <ChevronRight
                                size={16}
                                ...
                                >
                                  ...
                                  <div className="col-span-1">
                                    <button
                                      ...
                                      title={
                                        run.is_active
                                          ? "Versão ativa - clique para desativar"
                                          : "Clique para ativar esta versão"
                                      }
                                    >
                                      {toggling === run.id ? (
                                        <Loader2
                                    ...
                                    >
                                      {run.agent_status === "active"
                                        ...
                                        : run.agent_status ===
                                          ...
                                          : run.agent_status === "draft"
                                            ? "Rascunho"
                                            : run.agent_status === "superseded"
                                              ? "Substituído"
                                              : run.agent_status === "inactive"
                                                ? "Inativo"
