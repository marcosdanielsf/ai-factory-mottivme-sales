...
const BrandAdminList: React.FC<{ onSelectBrand: (brand: BrandConfig) => void }> = ({ onSelectBrand }) => {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {brands.map((brand) => {
        ...
        return (
          ...
          >
            {/* Color bar */}
            <div className="h-2" style={{ backgroundColor: brand.primary_color }} />
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-text-primary truncate">{brand.client_name}</p>
                  {brand.client_tagline && (
                    <p className="text-xs text-text-muted truncate">{brand.client_tagline}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1">
                {brand.sections.slice(0, 5).map((sec) => (
                  <span key={sec} className="px-2 py-0.5 text-[10px] font-medium bg-bg-tertiary text-text-muted rounded-full">
...
const ClientBrand: React.FC = () => {
  ...
  const [adminSelectedBrand, setAdminSelectedBrand] = useState<BrandConfig | null>(null);
    ...
    if (!adminSelectedBrand) {
      return (
        <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
          <div>
            <h1 className="text-xl font-bold text-text-primary">Brand Assets</h1>
            <p className="text-sm text-text-secondary mt-0.5">
              Identidade visual de todos os clientes
            </p>
          </div>
          <BrandAdminList onSelectBrand={(brand) => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-text-primary">
              {adminSelectedBrand && !brandConfig ? 'Brand Assets' : 'Meu Brand'}
            </h1>
            <p className="text-sm text-text-secondary mt-0.5">
