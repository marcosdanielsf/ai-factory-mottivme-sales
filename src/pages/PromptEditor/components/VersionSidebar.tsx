...
export function VersionSidebar({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default:
        return "text-accent-warning"; // sandbox / pending_approval
    }
  };
    ...
    <aside className="w-64 border-r border-border-default bg-bg-secondary flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        {!versionsLoading && versions.length === 0 && (
          <div className="text-center py-4 text-xs text-text-muted">
            Nenhuma versão encontrada para este agente.
          </div>
        )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center">
                          <div
                            className={`text-sm font-medium ${activeVersionId === v.id ? "text-text-primary" : "text-text-secondary"}`}
                          >
                            {v.version_number || v.version || "v?"}
                          </div>
                          <span className="text-[10px] bg-accent-success/20 text-accent-success px-1.5 py-0.5 rounded border border-accent-success/30">
                            Ativo
                          </span>
                        </div>
              ...
              {inactiveVersions.length > 0 && (
                <>
                  ...
                  {inactiveVersions.map((v: AgentVersion) => (
                    ...
                    >
                      <div
                        className={`text-xs ${getStatusColor(v.validation_status || v.status)}`}
                      >
                        {(v.validation_status || v.status) === "archived" ? (
                          <AlertCircle size={14} />
                        ) : (
                          <FileCode size={14} />
                        )}
