      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center p-8">
          ...
          <h2 className="text-lg font-bold text-[var(--text-primary)]">
            Erro ao renderizar Metrics Lab
          </h2>
...
const DeltaBadge: React.FC<DeltaBadgeProps> = ({
  ...
  return (
    <div
      className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-semibold tabular-nums ${
        good
...
const PeriodDeltasRow: React.FC<{ deltas: PeriodDeltas }> = ({ deltas }) => {
  const badges: {
    ...
    {
      value: deltas.impressions_delta,
      label: "Impress.",
      higherIsBetter: true,
    },
...
const AnomalyBanner: React.FC<{ anomalies: AnomalyRow[] }> = ({
  anomalies,
}) => {
  ...
  return (
    <div
      className={`mx-4 md:mx-6 my-2 rounded-xl overflow-hidden border ${
        isCritical
    ...
    >
      ...
      >
        <AlertTriangle
          size={13}
          className={isCritical ? "text-rose-400" : "text-amber-400"}
        />
        <span
          className={`text-[12px] font-semibold ${isCritical ? "text-rose-300" : "text-amber-300"}`}
        >
          {count}{" "}
          {count === 1 ? "anuncio com anomalia" : "anuncios com anomalia"}
        </span>
        <ChevronDown
          size={13}
            ...
            >
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {a.cpl_delta_pct !== 0 && (
                  ...
                  >
                    CPL {a.cpl_delta_pct > 0 ? "+" : ""}
                    {(a.cpl_delta_pct ?? 0).toFixed(0)}%
                  </span>
                )}
                {a.ctr_delta_pct !== 0 && (
                  <span
                  ...
                  >
                    CTR {a.ctr_delta_pct > 0 ? "+" : ""}
                    {(a.ctr_delta_pct ?? 0).toFixed(0)}%
                  </span>
                )}
              </div>
            </div>
...
export const MetricsLab: React.FC = () => {
  ...
  // Sync: fire-and-forget n8n webhook, then poll for fresh data
  const handleSync = async () => {
    if (syncing) return;
    syncAbortRef.current = false;
    setSyncing(true);
    try {
      // Fire-and-forget — workflow takes 30-60s, don't await response
      fetch("https://cliente-a1.mentorfy.io/webhook/metrics-lab-refresh", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: "metrics-lab" }),
      }).catch(() => {});
      // Poll: refetch every 10s for 60s to catch when n8n finishes
      for (let i = 0; i < 6; i++) {
  ...
  const selectedARC = useMemo(() => {
    ...
    return arc
      ? {
          hook_rate: arc.hook_rate,
          hold_rate: arc.hold_rate,
          body_rate: arc.body_rate,
        }
      : null;
  ...
  const selectedConversionTime = useMemo(
    () =>
      conversionTimeMap && selectedAdId
        ? (conversionTimeMap.get(selectedAdId) ?? null)
        : null,
    [conversionTimeMap, selectedAdId],
  );
      ...
      <div className="sticky top-0 z-20 bg-[var(--bg-primary)]/95 backdrop-blur-lg">
          ...
          <div className="flex items-center justify-between gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-xs text-[var(--text-secondary)]">
                    Ranking de anuncios + funil completo
                  </p>
                  {unattributedCount > 0 && (
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1.5 bg-white/[0.05] hover:bg-white/[0.08] disabled:opacity-40 rounded-xl px-3 py-2 text-[12px] text-[var(--text-primary)] transition-colors"
              >
                <RefreshCw
                  size={13}
                  className={syncing ? "animate-spin text-violet-400" : ""}
                />
              ...
              <div className="relative" ref={exportRef}>
                ...
                >
                  ...
                  <ChevronDown
                    size={11}
                    className={`text-[var(--text-secondary)] transition-transform ${exportOpen ? "rotate-180" : ""}`}
                  />
                </button>
                ...
                {exportOpen && (
                  <div className="absolute right-0 top-full mt-1.5 z-50 bg-[#1a1d24] border border-white/[0.08] shadow-xl rounded-xl overflow-hidden min-w-[120px]">
                    <button
                      onClick={() => {
                        exportCSV(leadScoreRows, funnelAds);
                        setExportOpen(false);
                      }}
                    ...
                    <button
                      onClick={() => {
                        exportPDF(leadScoreRows, funnelAds);
                        setExportOpen(false);
                      }}
                      className="w-full flex items-center gap-2 px-3 py-2.5 text-[12px] text-[var(--text-primary)] hover:bg-white/[0.06] transition-colors text-left"
                    >
        ...
        {error && (
          <div className="mx-4 md:mx-6 my-2 flex items-center gap-2 px-3 py-2 rounded-xl bg-rose-500/10 text-rose-300 text-xs">
            ...
            <span>
              Erro ao carregar dados reais — exibindo dados de exemplo. ({error}
              )
            </span>
          </div>
        )}
