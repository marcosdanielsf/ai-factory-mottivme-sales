...
// ==========================================
// LEADS x UTM TABLE - Visão planilha individual
// ==========================================
...
const LeadsUtmTable: React.FC<LeadsUtmTableProps> = ({ leads, loading, locationId }) => {
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState<string | null>(null);
  const [sortCol, setSortCol] = useState<'created_at' | 'first_name' | 'status'>('created_at');
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter((l) => {
        ...
        return name.includes(q) || email.includes(q) || phone.includes(q) || utm.includes(q);
      });
    }
  ...
  const SortIcon = ({ col }: { col: typeof sortCol }) => (
    <ChevronDown size={12} className={`inline ml-0.5 transition-transform ${sortCol === col ? 'text-accent-primary' : 'text-text-muted'} ${sortCol === col && sortAsc ? 'rotate-180' : ''}`} />
  );
  ...
  return (
    <div className="bg-bg-secondary border border-border-default rounded-lg p-3 md:p-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-3">
        <div>
          <h3 className="text-sm font-semibold text-text-primary">Leads x UTM (Individual)</h3>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        ...
        {Object.entries(statusCounts)
          .sort(([, a], [, b]) => b - a)
          .map(([label, count]) => {
            const stageStyle = Object.values(FUNNEL_STAGES).find((s) => s.label === label);
            const isActive = statusFilter === label;
            return (
              <button
                key={label}
                onClick={() => { setStatusFilter(isActive ? null : label); setPage(0); }}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border-default text-text-muted">
              <th className="text-left py-2 px-2 font-medium cursor-pointer hover:text-text-primary" onClick={() => handleSort('first_name')}>
              ...
              <th className="text-left py-2 px-2 font-medium cursor-pointer hover:text-text-primary" onClick={() => handleSort('status')}>
          ...
          <tbody>
            {paged.map((lead) => {
              ...
              const ghlUrl = locationId && lead.contact_id
                ...
                : null;
              return (
                <tr key={lead.id} className="border-b border-border-default/50 hover:bg-bg-hover/50 transition-colors">
                  <td className="py-2 px-2 text-text-primary font-medium max-w-[140px] truncate">{fullName}</td>
                  <td className="py-2 px-2 text-text-secondary max-w-[180px] truncate">{email || '—'}</td>
                  <td className="py-2 px-2 text-text-secondary whitespace-nowrap">{lead.phone ? formatPhone(lead.phone) : '—'}</td>
                  <td className="py-2 px-2 max-w-[160px] truncate" title={lead.utm_content || ''}>
                    {lead.utm_content ? (
                  ...
                  <td className="py-2 px-2 text-text-muted max-w-[140px] truncate" title={lead.utm_campaign || ''}>{lead.utm_campaign || '—'}</td>
                  ...
                  <td className="py-2 px-2">
                    {ghlUrl ? (
                      <a href={ghlUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300" title="Abrir no GHL">
                        <ExternalLink size={14} />
                      </a>
                    ) : (
                      <span className="text-text-muted">—</span>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-3 text-xs text-text-muted">
          <span>Pg {page + 1} de {totalPages}</span>
