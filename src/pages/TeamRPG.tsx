...
export const TeamRPG = () => {
    ...
    async (isRefresh = false) => {
      ...
      try {
        ...
        const loadedSquads = results
          .filter(
            (r): r is PromiseFulfilledResult<Squad & { clientId: string }> =>
              r.status === "fulfilled",
          )
          .map((r) => r.value);
        ...
        if (failedCount > 0) {
          console.warn(
            `[TeamRPG] ${failedCount} squad(s) falharam ao carregar`,
          );
          results
            .filter((r): r is PromiseRejectedResult => r.status === "rejected")
            .forEach((r, i) => console.error(`Squad ${i} error:`, r.reason));
          showToast(
            `${loadedSquads.length} squads carregados, ${failedCount} falharam`,
            "warning",
          );
        }

        setSquads(loadedSquads);
        if (isRefresh && failedCount === 0)
          showToast("Squads atualizados com sucesso", "success");
      } catch (error) {
  ...
  const handleUpdatePrompt = useCallback(
    (memberId: string, skillId: string, newContent: string) => {
      setSquads(
        produce((draft) => {
  ...
  if (!selectedClient) {
    return (
      <div className="p-8 min-h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-bg-secondary via-bg-primary to-bg-primary">
        ...
        <ClientRanking clients={clients} onSelectClient={setSelectedClient} />
      </div>
    );
  }
        ...
        {clientSquads.length > 0 ? (
            ...
            <div key={squad.id} className="mt-6">
              ...
              <div className="bg-bg-primary/50 border border-border-default rounded-xl p-4 sm:p-6 lg:p-8 relative overflow-visible min-h-[300px] flex items-end gap-4 sm:gap-8 lg:gap-12 overflow-x-auto pb-12 shadow-inner">
                ...
                {squad.members.map((member) => (
                  <div
                    key={member.id}
                    className="relative flex-shrink-0 group/avatar-container z-10 cursor-pointer hover:scale-105 transition-transform duration-200"
                  >
                    <Avatar
                      member={member}
                      onClick={(m) => {
          ...
          <div className="py-20 text-center text-text-muted border border-dashed border-border-default rounded-xl">
            ...
            <p className="font-medium text-text-primary mb-1">
              Nenhum squad designado
            </p>
            <p className="text-sm">
              Configure agentes para este cliente no painel admin.
            </p>
          </div>
        )}
      </div>
