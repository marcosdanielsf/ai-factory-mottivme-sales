...
import {
  ...
  NewFollower,
  AccountSummary,
} from '../src/hooks/useNewFollowers';
...
export const NewFollowers = () => {
  ...
  const { sendOutreach, skipFollower, bulkSendOutreach, bulkSkipFollowers, loading: actionLoading } = useOutreachActions();
  ...
  const handleAddAccount = async () => {
    ...
    if (result) {
      showToast(`Conta @${newUsername} adicionada`, 'success');
      setNewUsername('');
  ...
  const handleToggleOutreach = async (account: AccountSummary) => {
    ...
    if (result) {
      showToast(
        account.outreach_enabled ? 'Outreach desativado' : 'Outreach ativado',
        'info'
      );
    }
  };
  ...
  const handleSendOutreach = async () => {
    ...
    if (result) {
      showToast('DM enviada com sucesso', 'success');
  ...
  const handleSkipFollower = async (followerId: string) => {
    ...
    if (result) {
      showToast('Seguidor ignorado', 'info');
      refetchFollowers();
    }
  };
  ...
  const handleBulkOutreach = async () => {
    ...
    if (count > 0) {
      showToast(`DMs enviadas para ${count} seguidores`, 'success');
  ...
  const handleBulkSkip = async () => {
    ...
    if (count > 0) {
      showToast(`${count} seguidores ignorados`, 'info');
      setSelectedFollowers(new Set());
      refetchFollowers();
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    const badges: Record<string, { bg: string; text: string; label: string; icon: React.ReactNode }> = {
      ...
      skipped: { bg: 'bg-gray-500/10', text: 'text-gray-500', label: 'Ignorado', icon: <XCircle size={10} /> },
    ...
    <div className="max-w-7xl mx-auto p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border-default pb-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <UserPlus size={28} className="text-accent-primary" />
            <h1 className="text-3xl font-semibold">Novos Seguidores</h1>
          </div>
          <p className="text-text-secondary">Detecte e aborde novos seguidores automaticamente.</p>
        </div>

        <div className="flex items-center gap-3">
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-accent-primary text-white rounded-lg hover:bg-accent-primary/90 transition-all active:scale-95"
          >
            <Plus size={16} />
            Adicionar Conta
          </button>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loadingAccounts ? (
            [...Array(3)].map((_, i) => (
              <div key={i} className="bg-bg-secondary border border-border-default rounded-xl p-4 animate-pulse">
            ...
            accounts.map((account) => (
              <div
                key={account.id}
                className={`bg-bg-secondary border rounded-xl p-4 transition-all cursor-pointer hover:border-accent-primary/50 ${
                  selectedAccountId === account.id ? 'border-accent-primary ring-2 ring-accent-primary/20' : 'border-border-default'
                }`}
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-text-primary">@{account.username}</span>
                ...
                <div className="grid grid-cols-4 gap-2 text-center">
                  <div className="bg-bg-primary border border-border-default rounded-lg p-2">
                    <div className="text-lg font-bold text-text-primary">{account.total_new_followers}</div>
            ...
            <div className="col-span-full bg-bg-secondary border border-border-default rounded-xl p-8 text-center">
              <Instagram size={48} className="mx-auto text-text-muted opacity-20 mb-4" />
              <h3 className="text-lg font-medium text-text-primary mb-1">Nenhuma conta monitorada</h3>
              <p className="text-sm text-text-muted mb-4">Adicione uma conta Instagram para comecar a detectar novos seguidores.</p>
              <button
                onClick={() => setShowAddModal(true)}
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-accent-primary text-white rounded-lg hover:bg-accent-primary/90"
              >
                <Plus size={16} />
                Adicionar Conta
              </button>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          {selectedFollowers.size > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-text-muted">{selectedFollowers.size} selecionados</span>
              <button
                onClick={() => setShowOutreachModal(true)}
                className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium bg-accent-primary text-white rounded-lg hover:bg-accent-primary/90"
              >
                <Send size={12} />
                Enviar DMs
              </button>
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium bg-bg-tertiary text-text-secondary rounded-lg hover:bg-bg-hover"
              >
                <XCircle size={12} />
                Ignorar
              </button>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                className="w-full bg-bg-primary border border-border-default rounded-lg pl-10 pr-10 py-2.5 text-sm text-text-primary focus:outline-none focus:border-accent-primary focus:ring-1 focus:ring-accent-primary/20 transition-all shadow-sm"
              />
              {searchTerm && (
                <button
          ...
          <div className="divide-y divide-border-default">
            {loadingFollowers ? (
              [...Array(itemsPerPage)].map((_, i) => (
                <div key={i} className="grid grid-cols-12 gap-4 p-4 animate-pulse">
                  ...
                  <div className="col-span-2">
                    <div className="h-6 bg-bg-tertiary rounded-full w-12" />
                  </div>
                  <div className="col-span-2">
                    <div className="h-6 bg-bg-tertiary rounded-full w-20" />
                  </div>
                  <div className="col-span-3 flex justify-end">
                    <div className="h-8 bg-bg-tertiary rounded-md w-24" />
                  </div>
                </div>
              ))
            ) : followers.length > 0 ? (
              followers.map((follower) => (
                <div
                  key={follower.id}
                  className={`grid grid-cols-12 gap-4 p-4 items-center hover:bg-bg-hover transition-all group ${
                    selectedFollowers.has(follower.id) ? 'bg-accent-primary/5' : ''
                  }`}
                >
                  <div className="col-span-1">
                    <input
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <a
                          ...
                          className="font-bold text-text-primary hover:text-accent-primary flex items-center gap-1"
                      ...
                      {follower.follower_full_name && (
                        <p className="text-xs text-text-secondary truncate">{follower.follower_full_name}</p>
                      )}
                      {follower.follower_bio && (
                        <p className="text-[10px] text-text-muted truncate mt-0.5">{follower.follower_bio}</p>
                      )}
                    </div>
                  </div>
                    ...
                    {follower.outreach_status === 'pending' && (
                      <>
                        <button
                          ...
                          className="text-xs font-bold flex items-center gap-1 bg-accent-primary hover:bg-accent-primary/90 text-white transition-all px-3 py-1.5 rounded-lg shadow-sm disabled:opacity-50"
                        >
                          <Send size={12} />
                          Enviar DM
                        </button>
                        <button
                          ...
                          disabled={actionLoading}
                          className="text-xs font-medium flex items-center gap-1 bg-bg-tertiary hover:bg-bg-hover text-text-secondary transition-all px-3 py-1.5 rounded-lg disabled:opacity-50"
                        >
                          <XCircle size={12} />
                        </button>
                      </>
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-medium text-text-primary mb-1">Nenhum novo seguidor</h3>
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2">
            <div className="text-xs text-text-muted">
              Mostrando <span className="text-text-primary font-semibold">{followers.length}</span> de{' '}
              <span className="text-text-primary font-semibold">{totalCount}</span> seguidores
            </div>
            ...
            <div className="flex gap-2 items-center">
              <button
                ...
                className={`p-2 rounded-lg border border-border-default bg-bg-secondary text-text-primary transition-all shadow-sm ${
                  currentPage === 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-bg-hover'
                }`}
              >
                <ChevronLeft size={18} />
              </button>
              ...
              <button
                ...
                className={`p-2 rounded-lg border border-border-default bg-bg-secondary text-text-primary transition-all shadow-sm ${
                  currentPage === totalPages ? 'opacity-30 cursor-not-allowed' : 'hover:bg-bg-hover'
                }`}
              >
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-bg-secondary border border-border-default rounded-xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-border-default">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-text-primary">Adicionar Conta Instagram</h3>
                <button
                  onClick={() => setShowAddModal(false)}
                  className="p-2 hover:bg-bg-tertiary rounded-md text-text-muted"
                >
                  <X size={18} />
                </button>
              </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-2">Username do Instagram</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-bg-primary border border-border-default rounded-lg pl-8 pr-4 py-2.5 text-sm text-text-primary focus:outline-none focus:border-accent-primary"
                  />
                </div>
              </div>
            ...
            <div className="p-6 border-t border-border-default flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium bg-accent-primary text-white rounded-lg hover:bg-accent-primary/90 disabled:opacity-50"
              >
                Adicionar
              </button>
            </div>
      ...
      {showConfigModal && configAccount && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-bg-secondary border border-border-default rounded-xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-border-default">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-text-primary">Configuracoes - @{configAccount.username}</h3>
                <button
                  onClick={() => {
                    setShowConfigModal(false);
                    setConfigAccount(null);
            ...
            <div className="p-6 space-y-6">
              <div>
                <label className="flex items-center justify-between cursor-pointer">
                  <span className="text-sm font-medium text-text-primary">Outreach Automatico</span>
                  <button
                    onClick={() =>
                      setConfigAccount({
                        ...configAccount,
                    ...
                    className={`relative w-12 h-6 rounded-full transition-colors ${
                      configAccount.outreach_enabled ? 'bg-accent-primary' : 'bg-bg-tertiary'
                    }`}
                  >
                    <span
              ...
              <div>
                <label className="block text-sm font-medium text-text-primary mb-2">Limite Diario de DMs</label>
                <input
                  type="number"
                  min="1"
                  max="100"
                  ...
                  className="w-full bg-bg-primary border border-border-default rounded-lg px-4 py-2.5 text-sm text-text-primary focus:outline-none focus:border-accent-primary"
                />
                <p className="text-xs text-text-muted mt-1">
            ...
            <div className="p-6 border-t border-border-default flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium bg-accent-primary text-white rounded-lg hover:bg-accent-primary/90"
              >
                Salvar
              </button>
            </div>
      ...
      {showOutreachModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-bg-secondary border border-border-default rounded-xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-border-default">
              <div className="flex items-center justify-between">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Escreva sua mensagem de abordagem..."
                  rows={5}
                  className="w-full bg-bg-primary border border-border-default rounded-lg px-4 py-3 text-sm text-text-primary focus:outline-none focus:border-accent-primary resize-none"
                />
              </div>
