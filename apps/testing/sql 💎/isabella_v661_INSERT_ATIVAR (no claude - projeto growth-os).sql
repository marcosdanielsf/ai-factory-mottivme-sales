...
-- 1. Desativa versões anteriores
-- 2. Insere nova versão 6.6.1 com regras anti-loop
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- PASSO 2: INSERIR NOVA VERSÃO 6.6.1 ATIVA
-- ═══════════════════════════════════════════════════════════════════════════
...
$PROMPTS_JSON$,
  ...
  -- CONFIGS (JSON vazios ou padrão)
  '{}',  -- tools_config
  '{"max_tool_calls_per_conversation": {"Criar ou buscar cobranca": 1, "Busca_disponibilidade": 2, "Agendar_reuniao": 1}}',  -- compliance_rules
  '{"nome": "Isabella", "tom": "Elegante mas humana", "emoji_preferencial": "💜", "max_linhas": 4}',  -- personality_config
  '{"nome": "Instituto Amare", "doutor": "Dr. Luiz Augusto", "segmento": "Saúde Hormonal"}',  -- business_config
  '{}',  -- qualification_config
  '{}',  -- hyperpersonalization
  'v6.6.1 - Adicionadas regras anti-loop de ferramentas para evitar chamadas repetidas (bug crítico de 20x)',
  NOW(),
  NOW()
);
