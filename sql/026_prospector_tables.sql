...
-- INDEXES
CREATE INDEX IF NOT EXISTS idx_queue_campaign ON prospector_queue_leads(campaign_id);
CREATE INDEX IF NOT EXISTS idx_queue_channel ON prospector_queue_leads(channel);
CREATE INDEX IF NOT EXISTS idx_queue_stage ON prospector_queue_leads(stage);
CREATE INDEX IF NOT EXISTS idx_queue_next_action ON prospector_queue_leads(next_action_at);
CREATE INDEX IF NOT EXISTS idx_templates_channel ON dm_templates(channel);
CREATE INDEX IF NOT EXISTS idx_templates_stage ON dm_templates(stage);
CREATE INDEX IF NOT EXISTS idx_dm_logs_campaign ON prospector_dm_logs(campaign_id);
CREATE INDEX IF NOT EXISTS idx_dm_logs_sent ON prospector_dm_logs(sent_at);
CREATE INDEX IF NOT EXISTS idx_dm_logs_channel ON prospector_dm_logs(channel);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_dm_templates_updated') THEN
...
CREATE POLICY "Allow all for authenticated" ON dm_templates FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY "Service role full access" ON dm_templates FOR ALL TO service_role USING (true) WITH CHECK (true);
